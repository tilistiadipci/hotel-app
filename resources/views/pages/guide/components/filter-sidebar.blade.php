    ...
    <form id="filterForm" class="px-3 pt-2">
        <div class="form-group">
            ...
            <div class="d-flex align-items-center">
                <select id="filterCategory" class="form-control form-control-sm select2 filter-select" style="width: 100%;" data-placeholder="{{ trans('common.all') }}">
                    <option value="">{{ trans('common.all') }}</option>
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}">{{ $category->name }}</option>
                    @endforeach
                </select>
                <button type="button" class="btn btn-outline-secondary btn-sm ml-2 clear-select" data-target="#filterCategory" aria-label="Clear category">&times;</button>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="d-flex align-items-center">
                ...
                <button type="button" class="btn btn-outline-secondary btn-sm ml-2 clear-select" data-target="#filterStatus" aria-label="Clear status">&times;</button>
            </div>
        </div>
