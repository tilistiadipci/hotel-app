        ...
        <div class="app-page-title">
            <div class="page-title-wrapper">
                ...
                @include('templates.parts.breadcrumb', [
                    ...
                    'icon' => $icon,
                    'breadcrumbs' => [
                        ['href' => '#', 'label' => trans('common.place_category.title')],
                    ],
                ])

                <div class="page-title-actions">
                    @include('partials.buttons.btn-create-new', [
                        'url' => route('place-categories.create'),
                    ])
                </div>
            </div>
        </div>
                ...
                <div class="card mb-3">
                    <div class="card-header-tab card-header">
                        <div class="card-header-title font-size-lg text-capitalize font-weight-normal">
                            {{ trans('common.place_category.list_of_place_category') }}
                        </div>
...
@section('js')
    <script>
        function attachFilters(d) {
            // no filters for place categories
        }
        ...
        var columns = [
            ...
            },
            {
                ...
                name: 'name',
                render: function(data, type, row) {
                    let url = `{{ url('place-categories') }}/${row.uuid ?? row.id}/edit`
        ...
        var getUrl = "{{ route('place-categories.index') }}";
        var showUrl = "{{ route('place-categories.show', ':id') }}";
        var editUrl = "{{ route('place-categories.edit', ':id') }}";
        var destroyUrl = "{{ route('place-categories.destroy', ':id') }}";
        var scrollX = false;
