        ...
        <div class="app-page-title">
            <div class="page-title-wrapper">
                ...
                @include('templates.parts.breadcrumb', [
                    ...
                    'icon' => $icon,
                    'breadcrumbs' => [
                        ['href' => '#', 'label' => trans('common.movie_category.title')],
                    ],
                ])

                <div class="page-title-actions">
                    @include('partials.buttons.btn-create-new', [
                        'url' => route('movie-categories.create'),
                    ])
                </div>
            </div>
        </div>
                ...
                <div class="card mb-3">
                    <div class="card-header-tab card-header">
                        <div class="card-header-title font-size-lg text-capitalize font-weight-normal">
                            {{ trans('common.movie_category.list_of_movie_category') }}
                        </div>
...
@section('js')
    <script>
        function attachFilters(d) {
            // no filters for movie categories
        }
        ...
        var columns = [
            ...
            },
            {
                ...
                name: 'name',
                render: function(data, type, row) {
                    let url = `{{ url('movie-categories') }}/${row.uuid ?? row.id}/edit`
        ...
        var getUrl = "{{ route('movie-categories.index') }}";
        var showUrl = "{{ route('movie-categories.show', ':id') }}";
        var editUrl = "{{ route('movie-categories.edit', ':id') }}";
        var destroyUrl = "{{ route('movie-categories.destroy', ':id') }}";
        var scrollX = false;
