...
<form action="{{ $category ? route('movie-categories.update', $category->uuid ?? $category->id) : route('movie-categories.store') }}" method="POST">
    @csrf
    @if ($category)
        @method('PUT')
    @endif
    ...
    <div class="card-footer d-block text-right">
        <div class="row">
            @include('partials.forms.save-buttons', [
                'cancelUrl' => route('movie-categories.index'),
