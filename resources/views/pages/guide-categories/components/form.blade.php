...
<form action="{{ $category ? route('guide-categories.update', $category->uuid ?? $category->id) : route('guide-categories.store') }}" method="POST">
    @csrf
    @if ($category)
        @method('PUT')
    @endif
    ...
    <div class="card-footer d-block text-right">
        <div class="row">
            @include('partials.forms.save-buttons', [
                'cancelUrl' => route('guide-categories.index'),
