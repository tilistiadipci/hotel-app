...
@section('content')
    <div class="app-main__inner">
        ...
        <div class="app-page-title">
            <div class="page-title-wrapper">
                ...
                @include('templates.parts.breadcrumb', [
                    'title' => trans('common.place.title'),
                    'icon' => $icon,
                    'breadcrumbs' => [
                        ['href' => '#', 'label' => trans('common.place.title')],
                    ],
                ])

                <div class="page-title-actions">
                    @include('partials.buttons.btn-create-new', [
                        'url' => route('places.create'),
                    ])
                </div>
            </div>
        </div>
                ...
                <div class="card mb-3">
                    <div class="card-header-tab card-header">
                        <div class="card-header-title font-size-lg text-capitalize font-weight-normal">
                            {{ trans('common.place.list_of_place') }}
                        </div>
                        ...
                        <table style="width: 100%;" class="table table-hover nowrap table-striped table-bordered data-table">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ trans('common.place.address') }}</th>
                                    <th>{{ trans('common.place.distance') }} (km)</th>
        ...
        @include('pages.places.components.filter-sidebar')
    </div>
@endsection
    ...
    <script>
        ...
        var columns = [
            ...
            },
            {
                ...
                name: 'name',
                render: function(data, type, row) {
                    let url = `{{ url('places') }}/${row.uuid}/edit`
        ...
        var getUrl = "{{ route('places.index') }}";
        var showUrl = "{{ route('places.show', ':id') }}";
        var editUrl = "{{ route('places.edit', ':id') }}";
        var destroyUrl = "{{ route('places.destroy', ':id') }}";
        var scrollX = false;
