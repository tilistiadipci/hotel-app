                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover hotel-table mb-0"><thead><tr><th>Plan</th><th>Status</th><th>Header Key</th><th>Maks. Player</th><th>Maks. User</th><th>Mulai</th><th>Berakhir</th></tr></thead><tbody>
                            @forelse($hotel->licenses->sortByDesc('starts_at') as $license)
                                <tr><td>{{ $license->plan_code }}</td><td><span class="badge badge-{{ $license->isUsable() ? 'success' : 'secondary' }}">{{ ucfirst($license->status) }}</span></td><td><span class="badge badge-{{ $license->license_key_fingerprint ? 'success' : 'warning' }}">{{ $license->license_key_fingerprint ? 'X-Hotel-License siap' : 'Perlu generate key baru' }}</span></td><td>{{ $license->max_players ?? 'Tanpa batas' }}</td><td>{{ $license->max_users ?? 'Tanpa batas' }}</td><td>{{ $license->starts_at?->format('d/m/Y') ?? '-' }}</td><td>{{ $license->expires_at?->format('d/m/Y') ?? 'Selamanya' }}</td></tr>
